...
+ (void)setLabelSpacing:(CGFloat)spacing {
    [self sharedView].labelSpacing = spacing;
}
...
- (void)updateHUDFrame {
    ...
    CGFloat labelSpacing = self.labelSpacing;
    ...
    hudWidth = horizontalSpacing + MAX(labelWidth, contentWidth) + horizontalSpacing;
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(verticalSpacing, (self.minimumSize.height - contentHeight - labelSpacing - labelHeight) / 2.0f);
    ...
    // Label
    if(imageUsed || progressUsed) {
        centerY = CGRectGetMaxY(imageUsed ? self.imageView.frame : self.indefiniteAnimatedView.frame) + labelSpacing + labelHeight / 2.0f;
    } else {
        centerY = CGRectGetMidY(self.hudView.bounds);
    }
...
+ (NSTimeInterval)displayDurationForString:(NSString*)string {
    CGFloat minimum = MAX((float)string.length * 0.11 + 0.5, [self sharedView].minimumDismissTimeInterval);
...
- (void)setLabelSpacing:(CGFloat)labelSpacing {
    if (!_isInitializing) labelSpacing = labelSpacing;
}
